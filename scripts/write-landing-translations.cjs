...
// Usage: node scripts/write-landing-translations.js

const fs = require('fs');
const path = require('path');

const LANGS_DIR = path.join(__dirname, '..', 'public', 'lang');
...
const translations = {
  ro: {
    ...
    "faq": {
      ...
      "a1": "Da, complet. Fără reclame, fără abonament, fără card și fără donații. Proiectul e susținut prin muncă benevolă.",
  ...
  es: {
    ...
    "faq": {
      ...
      "a1": "Sí, por completo. Sin anuncios, sin suscripción, sin tarjeta y sin donaciones. El proyecto se sostiene con trabajo voluntario.",
  ...
  en: {
    ...
    "faq": {
      ...
      "a1": "Yes, completely. No ads, no subscription, no card, no donations. The project is sustained by volunteer work.",
      "q2": "Is my data safe?",
  ...
  zh: {
    ...
    "faq": {
      ...
      "title": "你在其他地方找不到的",
      "q1": "真的免费吗?",
      "a1": "是的,完全免费。无广告、无订阅、无信用卡、无捐赠。该项目由志愿者工作支持。",
      "q2": "我的数据安全吗?",
...
for (const [langCode, landingData] of Object.entries(translations)) {
  const filePath = path.join(LANGS_DIR, `${langCode}.json`);

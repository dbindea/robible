...
11. **No hay router.** Añadir una ruta implica tocar `Main.svelte` (detección), `bible-versions.js` (el path por idioma), `AppMenu.svelte` (navegación) y `generate-seo.mjs` (sitemap).
12. **La lectura no tiene voz, por decisión de producto**: es música + resaltado visual. `TtsPlayer` recibe `playlist` (lo que hay en pantalla), no un capítulo. `tts.service.js` existe pero no lo importa nadie.

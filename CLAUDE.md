...
| `npm test` | Suite con `node --test` (105 tests, sin dependencias). **En Windows el glob es obligatorio**: `node --test tests` a secas intenta cargar el directorio como módulo y falla |
...
2. **Tocar `public/sw.js` obliga a bumpear `CACHE_NAME`** (hoy `robible-v25`). Sin bump, los usuarios con la PWA instalada no reciben el cambio. `public/sw.js` es la única fuente de verdad de la versión de cache: no dupliques la constante en otro sitio.
...
22. **Las reglas de tema oscuro pueden pisar los estados de color.** `:global(html[data-theme='dark']) .icon-btn` tiene tres clases de especificidad y `.icon-btn--marked` sólo dos, así que en oscuro el icono marcado perdía su color y volvía al azul mientras el borde sí lo conservaba. Por eso esa regla lleva `:not(.icon-btn--marked)`. Regla de la casa: **cuando una acción tiene color propio, ese color sustituye al azul de estado activo; no se mezclan.** Al añadir cualquier estado de color, compruébalo en los dos temas — este fallo sólo se veía en oscuro.
